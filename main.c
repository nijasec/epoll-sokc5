			...
			while (s != NULL)
			{
				int del = 0;
					...
					if (c->protocol == IPPROTO_TCP)
					{
						...
						sprintf(log,"Protocol: %s IPVersion:%d Source Address:%s:%d --> Destination Address:%s:%d Send:%ldKb Recieved:%ldKb\n",(c->protocol==6)?"TCP":c->protocol,c->tcp.version,source,ntohs(c->tcp.source),dest,ntohs(c->tcp.dest),c->tcp.sent/1024,c->tcp.received/1024);
						...
						if (c->client == ev[i].data.fd)
						{
							...
							switch (c->tcp.socks5)
							{
								...
								case SOCKS5_HELLO:
									...
									handle_sock_command(c, epoll_fd);
						...
						else if (c->remote == ev[i].data.fd)
						{
							...
							else
							{
								...
								//	printf("sent success:%d", n);
									...
									if (events != c->ev_r.events)
									{

										c->ev_r.events = events;
										if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, c->remote, &c->ev_r) < 0)
										{
											c->tcp.remote_state = CLOSED;
											printf("epoll mod tcp error %d: %s", errno, strerror(errno));
										}
									}
...
void handle_sock_command(struct session *c, const int epollfd)
{
	uint8_t domain[256];
	uint8_t buf[5];
	int n, i, sock;
	...
	else
	{
		...
		//printf("portnumber:%u addr_len:%d",ntohs(c->tcp.dest),addr_Len);
	...
	if (servinfo->ai_family == AF_INET)
					{
						...
						c->tcp.daddr.ip4 = (struct sockaddr_in *)servinfo->ai_addr;
						c->tcp.version = 4;
						// printf("IP:%s\n",address);
					}
					else
					{
						...
						c->tcp.daddr.ip6 = *(struct in6_addr *) servinfo->ai_addr;
						c->tcp.version = 6;
					}
}
